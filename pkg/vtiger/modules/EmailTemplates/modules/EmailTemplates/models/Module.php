...
class EmailTemplates_Module_Model extends Vtiger_Module_Model
{
    ...
    public function saveRecord(Vtiger_Record_Model $recordModel)
    {
        ...
        if (!empty($recordId) && $systemtemplate) {
            $params = [decode_html($recordModel->get('templatename')), decode_html($recordModel->get('description')),
                $recordModel->get('module'), $recordModel->get('body'), 0, $systemtemplate, $templateid];
        } else {
            $params = [decode_html($recordModel->get('templatename')), decode_html($recordModel->get('subject')),
    ...
    public function getCustomMergeTags()
    {
        $customMergeTags = [
            ...
            ['Current Time', '$custom-currenttime$'],
            ['View in browser', "<a target='_blank' href='" . self::$BROWSER_MERGE_TAG . "'>View in browser</a>"],
            ['CRM Detail View Url', '$custom-crmdetailviewurl$'],
            ['Portal Detail View Url', '$custom-portaldetailviewurl$'],
            ['Site Url', '$custom-siteurl$'],
            ['Portal Url', '$custom-portalurl$']];
