...
class OutlookSyncServer extends SyncServer
{
    ...
    {
        return ['handlerclass' => 'OutlookVtigerCRMHandler',
    ...
    public function put($key, $element, $user)
    {
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record['crmid'])) {
                ...
                $lookupResult = $db->pquery('SELECT crmid,modifiedtime FROM vtiger_crmentity WHERE crmid=?', [$crmid[1]]);
            ...
            // End
                ...
                $lookupResult = $db->pquery('SELECT serverid,clientmodifiedtime FROM vtiger_wsapp_recordmapping WHERE appid=? AND clientid=?', [$appid, $clientRecordId]);
                if ($db->num_rows($lookupResult)) {
