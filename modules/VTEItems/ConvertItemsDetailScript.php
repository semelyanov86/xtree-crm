...
if(class_exists("Quoter_Module_Model")) {
    ...
    while ($row = $adb->fetchByAssoc($rs)) {
        ...
        $recordModel->save();
        $adb->pquery("UPDATE vtiger_vteitems SET `sequence` = ? WHERE vteitemid = ?", array($row["sequence_no"], $recordModel->getId()));
        if($row["level"]) {
            $adb->pquery("UPDATE vtiger_vteitems SET `level` = ? WHERE vteitemid = ?", array($row["level"], $recordModel->getId()));
        }
        if($row["section_value"]) {
            $adb->pquery("UPDATE vtiger_vteitems SET `section_value` = ? WHERE vteitemid = ?", array($row["section_value"], $recordModel->getId()));
        }
        if($row["running_item_value"]) {
            $adb->pquery("UPDATE vtiger_vteitems SET `running_item_value` = ? WHERE vteitemid = ?", array($row["running_item_value"], $recordModel->getId()));
        }
    }
...
function convertTotalItems($quoterModule)
{
    ...
    foreach ($totalSettings as $moduleName => $totalColumms) {
        ...
        $avaiableColumns = array();
        $rsBlockId = $adb->pquery("SELECT blockid FROM vtiger_blocks WHERE tabid = ? and blocklabel = 'LBL_ITEM_DETAILS'", array($tabId));
        $blockId = false;
        if(0 < $adb->num_rows($rsBlockId)) {
            ...
            if(preg_match("/^ctf_/", $fieldName)) {
                if(in_array($fieldName, $columns)) {
                    ...
                    if(!in_array($fieldName, $cfColumns)) {
                        $adb->pquery("ALTER TABLE " . $custom_table . " ADD " . $fieldName . " DECIMAL(25,8)", array());
                    }
                }
                $sqlUpdateBlock = "";
                if($blockId) {
                ...
                $adb->pquery("UPDATE vtiger_field SET tablename = '" . $custom_table . "', displaytype = 3, presence = 2, fieldlabel = '" . $setting["fieldLabel"] . "', columnname = '" . $fieldName . "' " . $sqlUpdateBlock . " WHERE tabid = " . $tabId . " and fieldname = '" . $fieldName . "'", array());
            }
        }
        if(!empty($avaiableColumns)) {
            $arrSqlUpdateComposition = array();
            $arrSqlDropComposition = array();
            ...
            $adb->pquery($sqlDropColumns, array());
        }
    }
