...
class VTEItems extends CRMEntity
{
     ...
     */
    public $customFieldTable = array("vtiger_vteitemscf", "vteitemid");
    /**
     * Mandatory for Saving, Include tables related to this module.
     */
    public $tab_name = array("vtiger_crmentity", "vtiger_vteitems", "vtiger_vteitemscf");
    /**
     * Mandatory for Saving, Include tablename and tablekey columnname here.
     */
    public $tab_name_index = array("vtiger_crmentity" => "crmid", "vtiger_vteitems" => "vteitemid", "vtiger_vteitemscf" => "vteitemid");
    /**
     * Mandatory for Listing (Related listview)
     */
    public $list_fields = array("Item Name" => array("vteitems", "productid"), "Related to" => array("vteitems", "related_to"), "Quantity" => array("vteitems", "quantity"), "List Price" => array("vteitems", "listprice"), "Total" => array("vteitems", "total"), "Net Price" => array("vteitems", "net_price"));
    public $list_fields_name = array("Item Name" => "productid", "Related to" => "related_to", "Quantity" => "quantity", "List Price" => "listprice", "Total" => "total", "Net Price" => "net_price");
    public $list_link_field = "productid";
    public $search_fields = array("Item Name" => array("vteitems", "productid"), "Related to" => array("vteitems", "related_to"), "Quantity" => array("vteitems", "quantity"), "List Price" => array("vteitems", "listprice"), "Total" => array("vteitems", "total"), "Net Price" => array("vteitems", "net_price"));
    public $search_fields_name = array("Item Name" => "productid", "Related to" => "related_to", "Quantity" => "quantity", "List Price" => "listprice", "Total" => "total", "Net Price" => "net_price");
    public $popup_fields = array("productid");
    public $sortby_fields = array();
    public $def_basicsearch_col = "productid";
    public $def_detailview_recname = "productid";
    public $required_fields = array();
    public $mandatory_fields = array("productid", "related_to");
    ...
    public function addRelations()
    {
        ...
        foreach ($listModules as $module) {
            ...
            $rscheck = $adb->pquery("SELECT * FROM vtiger_relatedlists WHERE tabid = ? AND related_tabid = ?", array($tabId, $relTabId));
            if($adb->num_rows($rscheck) == 0) {
                $relation_id = $adb->getUniqueID("vtiger_relatedlists");
                $sequence = 0;
                $result = $adb->pquery("SELECT max(sequence) as maxsequence FROM vtiger_relatedlists WHERE tabid=?", array($tabId));
                ...
                $adb->pquery("INSERT INTO vtiger_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence,actions) VALUES(?,?,?,?,?,?,?,?)", array($relation_id, $tabId, $relTabId, $function_name, $sequence, $label, $presence, $actions));
            }
        }
    }
    public function addDefaultModuleTypeEntity()
    {
        ...
        if($adb->num_rows($rs) == 0) {
            ...
            $adb->pquery("INSERT INTO `vtiger_ws_entity` (`id`, `name`, `handler_path`, `handler_class`, `ismodule`) VALUES (?, ?, ?, ?, ?);", array($entityId, "VTEItems", "include/Webservices/VtigerModuleOperation.php", "VtigerModuleOperation", "1"));
