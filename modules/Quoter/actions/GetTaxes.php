        ...
        if (empty($idList)) {
            ...
            if ($moduleName == 'PurchaseOrder') {
                foreach ($currencies as $currency) {
                    $listPriceValues[$currency['currency_id']] = $currency['conversionrate'] * (float) $recordModel->get('purchase_cost');
                }
            }
            $listPrice = (float) $recordModel->get('unit_price') * (float) $conversionRate;
            $info['defaultValue'] = [$recordId => ['id' => $recordId, 'name' => decode_html($recordModel->getName()), 'taxes' => $taxes, 'listprice' => $listPrice, 'listpricevalues' => $listPriceValues, 'description' => decode_html($recordModel->get('description')), 'quantityInStock' => $recordModel->get('qtyinstock')]];
            $info['customValue'] = [$recordId => $this->getCustomFieldValue($recordId, $setting, $type, $currencyId)];
            ...
            foreach ($idList as $id) {
                ...
                $listPrice = (float) $recordModel->get('unit_price') * (float) $conversionRate;
                $info['defaultValue'][] = [$id => ['id' => $id, 'name' => decode_html($recordModel->getName()), 'taxes' => $taxes, 'listprice' => $listPrice, 'listpricevalues' => $listPriceValues, 'description' => decode_html($recordModel->get('description')), 'quantityInStock' => $recordModel->get('qtyinstock')]];
                $info['customValue'][] = [$id => $this->getCustomFieldValue($id, $setting, $type, $currencyId)];
            }
            $response->setResult($info);
        }
        $response->emit();
                ...
                foreach ($block as $fieldModel) {
                    if ($type == 'ProductsPopup' && $fieldModel->getName() == $val['productField']) {
                        if ($fieldModel->get('uitype') == 71 || $fieldModel->get('uitype') == 72) {
                            ...
                            $fieldValue[$val['columnName']] = (float) $fieldModel->get('fieldvalue') * (float) $conversionRate;
