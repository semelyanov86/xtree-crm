...
class Quoter_SaveAjax_Action extends Vtiger_BasicAjax_Action
{
    ...
    public function saveQuoterSetting(Vtiger_Request $request)
    {
        ...
        if(!empty($currentModule)) {
            ...
            $columnsAvailable = array("module", "item_name", "quantity", "listprice", "tax_total", "total", "net_price", "discount_amount", "discount_percent", "comment", "total_fields", "section_setting", "tax_totalamount");
            ...
            $adb->pquery($sql, array($item_name, $qty, $list_price, $tax_total, $total, $net_price, $discount_amount, $discount_percent, $comment, $tax_totalamount, $currentModule));
            ...
            if(!empty($customColumn)) {
                ...
                foreach ($customColumn as $val) {
                    ...
                    if(in_array($val, $columns)) {
                        ...
                        $adb->pquery($sql, array(json_encode($custom_column_value), $currentModule));
                    } else {
                        array_push($columnsAvailable, $val);
            ...
            foreach ($columns as $val) {
                if(!in_array($val, $columnsAvailable)) {
                    ...
                    $adb->pquery("ALTER TABLE " . $tableName . " DROP COLUMN " . $val, array());
                    $adb->pquery("ALTER TABLE vtiger_vteitems DROP COLUMN " . $val, array());
                }
            }
        }
    ...
    public function removeCustomValue($customColumn, $module, $fieldType)
    {
        ...
        switch ($module) {
            case "Invoice":
                ...
                $adb->pquery($sql, array($fieldType));
                break;
            case "SalesOrder":
    ...
    public function saveTotalFieldSetting(Vtiger_Request $request)
    {
        ...
        if(!empty($currentModule)) {
            ...
            $adb->pquery("UPDATE " . $tableName . " SET total_fields =? WHERE module = ?", array(serialize($data), $currentModule));
        }
        $response = new Vtiger_Response();
        $response->setResult(array());
        $response->emit();
    ...
    public function saveSectionValuesSetting(Vtiger_Request $request)
    {
        ...
        if(!empty($values) && !empty($currentModule)) {
            try {
                ...
                $adb->pquery("UPDATE " . $tableName . " SET section_setting =? WHERE module = ?", array(serialize($values), $currentModule));
                if(!empty($oldValue)) {
                    $query = $adb->pquery("SELECT `total_fields` FROM " . $tableName . " WHERE `module` = ?", array($currentModule));
                        ...
                        foreach ($totalSettings as $key => $val) {
                            foreach ($oldValue as $index => $section) {
                                if($section["oldVal"] == $val["sectionInfo"] && $val["sectionInfo"] != "") {
                                    ...
                                    $listRecordSection = $adb->pquery("SELECT  vte.vteitemid FROM `vtiger_vteitems` AS vte\r\n                                    INNER JOIN vtiger_crmentity as crm ON vte.related_to = crm.crmid\r\n                                    WHERE crm.setype = ? and vte.section_value = ?", array($currentModule, $section["oldVal"]));
                                    if(0 < $adb->num_rows($listRecordSection)) {
                                        while ($row = $adb->fetchByAssoc($listRecordSection)) {
                                            $adb->pquery("update `vtiger_vteitems` set `section_value` = ? WHERE section_value = ? AND `vteitemid` = ?", array($section["newVal"], $section["oldVal"], $row["vteitemid"]));
                                        }
                                    }
                                }
                            }
                        }
                        $adb->pquery("UPDATE " . $tableName . " SET total_fields =? WHERE module = ?", array(serialize($totalSettings), $currentModule));
                    }
                }
                $success = true;
            } catch (Exception $e) {
        ...
        $response = new Vtiger_Response();
        $response->setResult(array("success" => $success, "error" => $mes));
        $response->emit();
    }
        ...
        if(!empty($sectionValue) && !empty($currentModule)) {
            try {
                ...
                if(!empty($sectionValue)) {
                    $query = $adb->pquery("SELECT `section_setting`, `total_fields` FROM " . $tableName . " WHERE `module` = ?", array($currentModule));
                    if(0 < $adb->num_rows($query)) {
                        ...
                        $adb->pquery("UPDATE " . $tableName . " SET section_setting =? WHERE module = ?", array(serialize($sectionSettings), $currentModule));
                        $totalSettings = $adb->query_result($query, 0, "total_fields");
                        $totalSettings = unserialize(decode_html($totalSettings));
                        foreach ($totalSettings as $key => $val) {
