...
class Quoter_Record_Model extends Inventory_Record_Model
{
    ...
    public function getAssociatedProductsPSTemplates($module, $focus, $seid = "", $refModuleName = false)
    {
        ...
        $hdnDiscountPercent = (double) $focus->column_fields["hdnDiscountPercent"];
        $hdnDiscountAmount = (double) $focus->column_fields["hdnDiscountAmount"];
        ...
        foreach ($taxDetails as $taxId => $taxInfo) {
            ...
            if($compoundOn) {
                ...
                foreach ($compoundOn as $id) {
                    $amount = (double) $amount + (double) $taxDetails[$id]["amount"];
                }
                $taxAmount = (double) $amount * (double) $taxInfo["percentage"] / 100;
