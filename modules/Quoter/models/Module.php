...
{
    public $listSettingTable = ['quoter_quotes_settings', 'quoter_invoice_settings', 'quoter_salesorder_settings', 'quoter_purchaseorder_settings'];
    ...
    {
        $settingsLinks[] = ['linktype' => 'MODULESETTING', 'linklabel' => 'Settings', 'linkurl' => 'index.php?module=Quoter&parent=Settings&view=Settings', 'linkicon' => ''];
        $settingsLinks[] = ['linktype' => 'MODULESETTING', 'linklabel' => 'Uninstall', 'linkurl' => 'index.php?module=Quoter&parent=Settings&view=Uninstall', 'linkicon' => ''];
    ...
    public function getSettings()
    {
        ...
        foreach ($listTable as $table) {
            $rs = $adb->pquery('SELECT * FROM ' . $table, []);
            if ($adb->num_rows($rs) > 0) {
                ...
                if (!empty($settings[$module])) {
                    usort($settings[$module], static function ($a, $b) {
                        if ($a->index == $b->index) {
                            return 0;
                        }
    ...
    public function getCustomColumnValue($record, $productid, $num, $setting)
    {
        ...
        $rs = $adb->pquery('SELECT ' . $strFields . ' FROM vtiger_vteitems WHERE related_to = ? AND  productid = ? AND sequence = ?', [$record, $productid, $num]);
        if ($adb->num_rows($rs) > 0) {
    ...
    public function getTotalFieldsSetting($module)
    {
        ...
        $listTable = $this->listSettingTable;
        if (in_array($tableName, $listTable)) {
            $rs = $adb->pquery('SELECT total_fields FROM ' . $tableName . ' WHERE module = ?', [$module]);
            if ($adb->num_rows($rs) > 0) {
    ...
    public function getAllTotalFieldsSetting()
    {
        ...
        $listTable = $this->listSettingTable;
        foreach ($listTable as $table) {
            $rs = $adb->pquery('SELECT total_fields,module FROM ' . $table, []);
            if ($adb->num_rows($rs) > 0) {
    ...
    public function getAllSectionsSetting()
    {
        ...
        $listTable = $this->listSettingTable;
        foreach ($listTable as $table) {
            $rs = $adb->pquery('SELECT section_setting, module FROM ' . $table, []);
            if ($adb->num_rows($rs) > 0) {
                $moduleName = $adb->query_result($rs, 0, 'module');
    ...
    public function updateParentProduct($record)
    {
        ...
        if ($adb->num_rows($rs) > 0) {
            ...
            $adb->pquery('UPDATE vtiger_products SET unit_price = ? WHERE productid = ?', [$totalPrice, $record]);
            $rs1 = $adb->pquery('SELECT productid FROM vtiger_seproductsrel WHERE crmid = ?', [$record]);
    ...
    {
        global $adb;
        $rs = $adb->pquery('SELECT productid FROM vtiger_seproductsrel WHERE crmid = ?', [$record]);
        if ($adb->num_rows($rs) > 0) {
            ++$level;
