...
class Quoter extends CRMEntity
{
    ...
    {
        global $adb;
        $adb->pquery('DELETE FROM `vte_modules` WHERE module=?;', ['Quoter']);
        $adb->pquery('INSERT INTO `vte_modules` (`module`, `valid`) VALUES (?, ?);', ['Quoter', '0']);
    }
    ...
    public function changeGrandTotalWhenUpdate()
    {
        ...
        foreach ($listModules as $moduleName) {
            $tableName = 'quoter_' . strtolower($moduleName) . '_settings';
            $totalResult = $adb->pquery('SELECT total_fields FROM ' . $tableName, []);
            if ($adb->num_rows($totalResult) > 0) {
                ...
                if (empty($totalSetting)) {
                    ...
                    $adb->pquery('UPDATE ' . $tableName . ' SET total_fields = ?', [serialize($arrDefaultSettingTotal)]);
                } else {
                    ...
                    $adb->pquery('UPDATE ' . $tableName . ' SET total_fields = ?', [serialize($totalSetting)]);
                }
            }
        }
    }
    ...
    {
        $fields = ['subtotal' => ['fieldLabel' => 'LBL_ITEMS_TOTAL', 'fieldFormula' => 'SUM($net_price$)', 'fieldType' => 0, 'isDefault' => 1, 'isActive' => 1], 'discount_percent' => ['fieldLabel' => 'LBL_TOTAL_DISCOUNT_PERCENT', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1], 'discount_amount' => ['fieldLabel' => 'LBL_TOTAL_DISCOUNT_AMOUNT', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 0], 's_h_amount' => ['fieldLabel' => 'LBL_SHIPPING_HANDLING_CHARGES', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1], 'pre_tax_total' => ['fieldLabel' => 'LBL_PRE_TAX_TOTAL', 'fieldFormula' => '($subtotal$-$discount_amount$-($subtotal$*$discount_percent$/100)+$s_h_amount$)-SUM($tax_totalamount$)', 'fieldType' => 0, 'isDefault' => 1, 'isActive' => 1], 'tax' => ['fieldLabel' => 'LBL_TAX', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1], 's_h_percent' => ['fieldLabel' => 'LBL_TAXES_FOR_SHIPPING_AND_HANDLING', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1], 'adjustment' => ['fieldLabel' => 'LBL_ADJUSTMENT', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1], 'total' => ['fieldLabel' => 'LBL_GRAND_TOTAL', 'fieldFormula' => '$pre_tax_total$+$s_h_percent$+($adjustment$)+$tax$+SUM($tax_totalamount$)', 'fieldType' => 0, 'isDefault' => 1, 'isActive' => 1]];
        if ($module == 'Invoice') {
            $fields['received'] = ['fieldLabel' => 'LBL_RECEIVED', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1];
            $fields['balance'] = ['fieldLabel' => 'LBL_BALANCE', 'fieldFormula' => '$total$-$received$', 'fieldType' => 0, 'isDefault' => 1, 'isActive' => 1];
        } elseif ($module == 'PurchaseOrder') {
            $fields['paid'] = ['fieldLabel' => 'LBL_PAID', 'fieldFormula' => '', 'fieldType' => 1, 'isDefault' => 1, 'isActive' => 1];
            $fields['balance'] = ['fieldLabel' => 'LBL_BALANCE', 'fieldFormula' => '$total$-$paid$', 'fieldType' => 0, 'isDefault' => 1, 'isActive' => 1];
        }
    ...
    private function addWidgetTo()
    {
        ...
        $adb->pquery('INSERT INTO `vtiger_settings_field` (`fieldid`, `blockid`, `name`, `description`, `linkto`, `sequence`) VALUES (?, ?, ?, ?, ?, ?)', [$max_id, $blockid, 'Item Details Customizer (Advanced)', 'Settings area for Quoter', 'index.php?module=Quoter&parent=Settings&view=Settings', $max_id]);
        ...
        foreach ($listModules as $moduleName) {
            ...
            if ($moduleName == 'PurchaseOrder') {
                ...
                if ($adb->num_rows($rs) > 0) {
                    ...
                    $adb->pquery('UPDATE `quoter_purchaseorder_settings` set `listprice` = ?', [$data]);
                }
            }
