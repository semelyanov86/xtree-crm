...
class QuoterHandler extends VTEventHandler
{
    public function handleEvent($eventName, $entityData)
    {
        if ($eventName == 'vtiger.entity.beforesave') {
            $modulesHandle = ['Quotes', 'Invoice', 'SalesOrder', 'PurchaseOrder', 'PSTemplates'];
        ...
        if ($eventName == 'vtiger.entity.aftersave') {
            ...
            if (in_array($currentModule, $modulesHandle)) {
                ...
                $res = $adb->pquery('SELECT taxname, taxlabel FROM vtiger_inventorytaxinfo WHERE deleted = 0', []);
                $taxTypes = [];
                if ($adb->num_rows($res) > 0) {
                    $blocks = ['LBL_ITEM_DETAILS'];
                    $fields = [];
                    for ($i = 0; $i < $adb->num_rows($res); ++$i) {
                        $taxname = $adb->query_result($res, $i, 'taxname');
                        $taxlabel = $adb->query_result($res, $i, 'taxlabel');
                        $taxTypes[] = $taxname;
                        $fields['LBL_ITEM_DETAILS'][$taxname] = ['label' => $taxlabel, 'uitype' => 9, 'typeofdata' => 'N~O'];
                    }
                ...
                if (isset($entityData->focus->_recurring_mode) && $entityData->focus->_recurring_mode == 'recurringinvoice_from_so' && isset($entityData->focus->_salesorderid) && $entityData->focus->_salesorderid != '') {
                        ...
                        while ($row = $adb->fetchByAssoc($res)) {
                            ...
                            if ($row['vteitemid'] && !in_array('VTEItems', $nonEntityModules)) {
                                ...
                                if ($recordEntityName === 'VTEItems') {
                                    ...
                                    $adb->pquery($sql, [$row['vteitemid'], $vteItemRecordModel->getId()]);
                                }
                            }
                        }
                    }
                    ...
                    if ($adb->num_rows($rs) > 0) {
                        while ($row = $adb->fetchByAssoc($rs)) {
                            ...
                            $params2 = [$adjustment, $s_h_amount, $s_h_percent, $pre_tax_total, $record];
                            $adb->pquery($sql2, $params2);
                            $adb->pquery('UPDATE `vtiger_invoicecf` SET `tax`=? WHERE `invoiceid`=?', [$tax, $record]);
                            $entityData->set('hdnS_H_Percent', $s_h_percent);
                            $entityData->set('tax', $tax);
                        }
                    }
                    $this->handleEventForRecurringInvoice($eventName, $entityData);
                } elseif ($totalProductCount > 0) {
                    $adb->pquery("UPDATE vtiger_vteitems SET  `running_item_value` = '', `section_value` = '' WHERE related_to = ?", [$record]);
                    ...
                    $adb->pquery('DELETE FROM vtiger_inventorysubproductrel WHERE id =?', [$record]);
                    unset($setting['item_name']);
                    $uitypeSeven = ['listprice', 'discount_amount', 'purchase_cost', 'margin', 'total', 'net_price', 'tax_totalamount'];
                    for ($i = 1; $i <= $totalProductCount; ++$i) {
                        ...
                        $adb->pquery('UPDATE vtiger_vteitems SET `sequence` = ? WHERE vteitemid = ?', [$i, $recordModel->getId()]);
                        $level = $_REQUEST['level' . $i];
                        $adb->pquery('UPDATE vtiger_vteitems SET `level` = ? WHERE productid =? AND related_to = ? AND sequence = ?', [$level, $productId, $record, $i]);
                        $parentId = vtlib_purify($_REQUEST['parentProductId' . $i]);
                        if (!empty($parentId)) {
                            $adb->pquery('INSERT INTO vtiger_inventorysubproductrel(id,sequence_no,productid) VALUES (?,?,?)', [$record, $i, $parentId]);
                        }
                        if (isset($_REQUEST['section' . $i]) && !empty($_REQUEST['section' . $i])) {
                            $section = vtlib_purify($_REQUEST['section' . $i]);
                            $adb->pquery('UPDATE vtiger_vteitems SET `section_value` = ? WHERE productid = ? AND related_to = ? AND sequence = ?', [$section, $productId, $record, $i]);
                            if ($targetModule) {
                                $tableName = 'quoter_' . strtolower($targetModule) . '_settings';
                                $result = $adb->pquery('SELECT section_setting FROM ' . $tableName . ' WHERE module = ?', [$targetModule]);
                                if ($adb->num_rows($result)) {
                                    $section_setting = decode_html($adb->query_result($result, 0, 'section_setting'));
                                    $unserializeSectionSetting = [];
                                    if (isset($section_setting) && $section_setting != '') {
                                        ...
                                        if (!in_array($section, $unserializeSectionSetting)) {
                                            ...
                                            $adb->pquery($query, [$serializeSectionSetting, $targetModule]);
                                        }
                                    } else {
                                        $unserializeSectionSetting[] = $section;
                                        $serializeSectionSetting = serialize($unserializeSectionSetting);
                        ...
                        if (isset($_REQUEST['running_item_name' . $i]) && !empty($_REQUEST['running_item_name' . $i]) && is_array($_REQUEST['running_item_name' . $i])) {
                            ...
                            $adb->pquery('UPDATE vtiger_vteitems SET  `running_item_value` = ? WHERE productid = ? AND related_to = ? AND sequence = ?', [$strValue, $productId, $record, $i]);
                        }
                    }
                    $arrTotalValue = [];
                    $arrTotalFieldName = [];
                    ...
                    if ($currentModule == 'PurchaseOrder') {
                        ...
                        if ($isNew && $data['postatus'] === 'Received Shipment') {
                            ...
                            $result = $adb->pquery('SELECT productid, quantity FROM vtiger_inventoryproductrel WHERE id = ?', [$record]);
                        ...
                        $adb->pquery('UPDATE vtiger_inventoryproductrel SET incrementondel=1 WHERE id=?', [$record]);
                        $product_info = $adb->pquery('SELECT productid,sequence_no, quantity from vtiger_inventoryproductrel WHERE id=?', [$record]);
                        ...
                        for ($index = 0; $index < $numrows; ++$index) {
                            ...
                            $sub_prod_query = $adb->pquery('SELECT productid from vtiger_inventorysubproductrel WHERE id=? AND sequence_no=?', [$record, $sequence_no]);
                ...
                if (!empty($sourceRecord)) {
                    global $adb;
                    $rs = $adb->pquery('SELECT * FROM vtiger_products WHERE productid =?', [$sourceRecord]);
                    if ($adb->num_rows($rs)) {
        ...
        if ($adb->num_rows($rs) > 0) {
            while ($row = $adb->fetchByAssoc($rs)) {
                ...
                if ($row['section_value']) {
                    $adb->pquery('UPDATE vtiger_vteitems SET `section_value` = ? WHERE vteitemid = ?', [$row['section_value'], $vteitemid]);
                }
                if ($row['running_item_value']) {
                    $adb->pquery('UPDATE vtiger_vteitems SET `running_item_value` = ? WHERE vteitemid = ?', [$row['running_item_value'], $vteitemid]);
                }
            }
            if (!empty($totalFieldsSetting)) {
                ...
                $rs2 = $adb->pquery($sql2, [$tabId]);
                if ($adb->num_rows($rs2) > 0) {
                    while ($row2 = $adb->fetchByAssoc($rs2)) {
                        $fieldname = $row2['fieldname'];
                        $tablename = $row2['tablename'];
                        $columnname = $row2['columnname'];
                        $fieldNames[$columnname] = ['fieldname' => $fieldname, 'tablename' => $tablename];
                    }
                }
                $sqls = [];
                $totalFieldsSettingRemain = [];
                foreach ($totalFieldsSetting as $fieldName => $fieldSetting) {
                ...
                foreach ($totalFieldsSetting as $fieldName => $fieldSetting) {
                    if ($fieldSetting['is_calculated']) {
                        ...
                        $param = [$fieldSetting['new_value'], $record];
    ...
    public function checkRecordExisted($crmid, $productid, $seq)
    {
        ...
        $rs = $adb->pquery($sql, [$crmid, $productid, $seq]);
        if ($adb->num_rows($rs) > 0) {
...
function caculateSUMorAVGThenMergeToFormula($formula, $vteitems)
{
    ...
    preg_match_all($pattern, $formula, $matches, PREG_OFFSET_CAPTURE);
    if (!empty($matches)) {
        [$sum_formula_arr, $inside_sum_formula_arr] = $matches;
    ...
    preg_match_all($pattern, $formula, $matches, PREG_OFFSET_CAPTURE);
    if (!empty($matches)) {
        [$avg_formula_arr, $inside_avg_formula_arr] = $matches;
