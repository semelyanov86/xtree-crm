...
class Quoter_MassActionAjax_View extends Vtiger_IndexAjax_View
{
    ...
    public function getItemsDetail(Vtiger_Request $request)
    {
        ...
        if(!empty($setting)) {
            ...
            $columnDefault = array("item_name", "quantity", "listprice", "tax_total", "total", "net_price", "comment", "discount_amount", "discount_percent", "tax_totalamount");
            ...
            $response->setResult(array("isActive" => true, "html" => $viewer1->view("LineItemsDetail.tpl", $moduleName, true), "html1" => $itemResultViewer->view("LineItemResult.tpl", $moduleName, true)));
            $response->emit();
        } else {
            $response = new Vtiger_Response();
            $response->setResult(array("isActive" => false, "html" => ""));
            $response->emit();
        }
    }
    public function getItemsEdit(Vtiger_Request $request)
    {
        ...
        if(!empty($setting)) {
            ...
            $response = new Vtiger_Response();
            $response->setResult(array("isActive" => true, "timeTrackerStatus" => $timeTrackerStatus, "html" => $lineItemViewer->view("LineItemsEdit.tpl", $moduleName, true), "setting" => $settingNotOrder, "html1" => $itemResultViewer->view("LineItemResult.tpl", $moduleName, true), "html2" => $lineItemViewer->view("CurrencyRate.tpl", $moduleName, true), "totalSettings" => $totalSettings, "sectionSettings" => $sectionSetting, "separator" => array("currency_grouping_separator" => $currency_grouping_separator, "currency_decimal_separator" => $currency_decimal_separator)));
            $response->emit();
        } else {
            $response = new Vtiger_Response();
            $response->setResult(array("isActive" => false, "html" => ""));
    ...
    private function getTemplatesList($module)
    {
        ...
        $res_1 = $adb->pquery($sql_1, array(0, $module));
        $templates_list = array();
        if($adb->num_rows($res_1)) {
            while ($row_1 = $adb->fetch_array($res_1)) {
                $templates_list[] = array("id" => $row_1["pstemplatesid"], "name" => $row_1["pstemplatename"]);
            }
        }
