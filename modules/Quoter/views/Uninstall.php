...
class Quoter_Uninstall_View extends Settings_Vtiger_Index_View
{
    ...
    public function removeData()
    {
        ...
        $adb->pquery('DELETE FROM vtiger_settings_field WHERE `name` = ?', ['Quoter']);
        $adb->pquery('DELETE FROM vtiger_fieldmodulerel WHERE module = ?', ['VTEItems']);
        $adb->pquery('DELETE FROM vtiger_modtracker_tabs WHERE tabid = ?', [$tabId]);
        $adb->pquery('DELETE FROM `vtiger_relatedlists` WHERE related_tabid = ?', [$tabId]);
