...
class PDFMaker_PDFContentUtils_Model extends Vtiger_Base_Model
{
    ...
    public static function controlInventoryModule($module)
    {
        $isInvModule = false;
    ...
    {
        if (is_numeric(strpos($content, '[BARCODE|'))) {
            $barcodes = explode('[BARCODE|', $content);

            foreach ($barcodes as $barcode) {
                $barcode = explode('|BARCODE]', $barcode)[0];

                $plaintext = str_replace('&amp;', '&', $barcode);
                $params = explode('|', $plaintext);
                [$type, $code] = explode('=', $params[0], 2);
                $barcodeAttrs = 'code="' . $code . '" type="' . $type . '" ';

                for ($i = 1; $i < PDFMaker_Utils_Helper::count($params); ++$i) {
                    [$attName, $attVal] = explode('=', $params[$i], 2);

                    $barcodeAttrs .= strtolower($attName) . '="' . $attVal . '" ';
                }

                $regex = '[BARCODE|' . $barcode . '|BARCODE]';
                $content = str_replace($regex, '<barcode ' . $barcodeAttrs . '/>', $content);
            }
        }
    ...
    public function getInventoryBreaklines($id)
    {
        ...
        $result = $adb->pquery(
            'SELECT productid, sequence, show_header, show_subtotal FROM vtiger_pdfmaker_breakline WHERE crmid=?',
            [$id],
        );
        $products = [];
        $showHeader = 0;
        ...
        while ($row = $adb->fetchByAssoc($result)) {
            ...
            $showHeader = (int) $row['show_header'];
            $showSubTotal = (int) $row['show_subtotal'];
        }

        $output['products'] = $products;
        $output['show_header'] = $showHeader;
        $output['show_subtotal'] = $showSubTotal;
    ...
    public function getUserImage($id)
    {
        if (isset($id) and $id != '') {
            $db = PearDatabase::getInstance();
            $result = $db->pquery('select vtiger_attachments.* from vtiger_attachments left join vtiger_salesmanattachmentsrel on vtiger_salesmanattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid where vtiger_salesmanattachmentsrel.smid=?', [$id]);
            $row = $db->query_result_rowdata($result);
            $row = PDFMaker_Module_Model::fixStoredName($row);
            $imgPath = $row['path'] . $row['attachmentsid'] . '_' . $row['storedname'];
    ...
    public function getInventoryCurrencyInfoCustomArray($inventory_table, $inventory_id, $id)
    {
        ...
        if (!empty($id)) {
            if (!empty($inventory_table)) {
                $sql = sprintf(
                    'SELECT currency_id, %s.conversion_rate AS conv_rate, vtiger_currency_info.* FROM %s
                    INNER JOIN vtiger_currency_info ON %s.currency_id = vtiger_currency_info.id WHERE %s=?',
                    $inventory_table,
                    $inventory_table,
                    $inventory_table,
                    $inventory_id,
     ...
     * @return string
     */
    public function getProductImage($id, $site_url, $productId = false, $sequence = '1')
    {
        ...
        [$images, $bacImages] = $this->getInventoryImages($id, $isProductModule);

        $imageKey = $productId . '_' . $sequence;

        if (isset($images[$imageKey])) {
            $image = $images[$imageKey];
        } elseif (isset($bacImages[$imageKey])) {
            $image = $bacImages[$imageKey];
        }

        if (method_exists($this, 'getSettings')) {
            $settings = (array) $this->getSettings();

            if (!isset($image['width']) || empty((int) $image['width'])) {
                $image['width'] = $settings['product_image_width'];
            }
            if (!isset($image['height']) || empty((int) $image['height'])) {
                $image['height'] = $settings['product_image_height'];
            }
        }

        if (isset($image['src']) && !empty($image['src'])) {
            $retImage = "<img src='" . $site_url . '/' . $image['src'] . "' ";
                ...
                if ($attid != '') {
                    if (isset($saved_products[$productid . '_' . $seq])) {
                        if ($saved_products[$productid . '_' . $seq] == $attid) {
                            $mainImgs[$productid . '_' . $seq] = $saved_wh[$productid . '_' . $seq];
    ...
    {
        $db = PearDatabase::getInstance();
        $res2 = $db->pquery('SELECT * FROM vtiger_crmentity WHERE crmid = ?', [$id]);
            ...
            if (in_array($fieldname, $UITypes['userorotherfields']) || in_array($fieldname, $UITypes['userfields'])) {
                ...
                $field_res = $db->pquery('SELECT fieldid FROM  vtiger_field WHERE tabid=('
                    . 'SELECT tabid FROM  vtiger_tab WHERE name =
                                        (SELECT setype FROM vtiger_crmentity WHERE crmid = ?) ) AND columnname = ?', [$efocus->id, $culumnname]);
                ...
                if ($efocus->id == $id) {
                    ...
                    if ($referenceModuleName) {
                        $field_res = $db->pquery('SELECT fieldid FROM vtiger_field WHERE tablename=(SELECT tablename FROM vtiger_entityname WHERE modulename=?) '
                            . 'AND fieldname=(SELECT fieldname FROM vtiger_entityname WHERE modulename=?)', [$referenceModuleName, $referenceModuleName]);
                        $fieldid = $db->query_result($field_res, 0, 'fieldid');
                    }
                } else {
                    if ($efocus->id) {
                        $field_res = $db->pquery('SELECT fieldid FROM vtiger_field WHERE tabid=(SELECT tabid FROM vtiger_tab WHERE name=(SELECT setype FROM vtiger_crmentity WHERE crmid = ?) ) '
                            . 'AND fieldname=?', [$efocus->id, $fieldname]);
                        $fieldid = $db->query_result($field_res, 0, 'fieldid');
                        $relid = $efocus->id;
                    }
                }
        ...
        if (isset($UITypes['related']) && PDFMaker_Utils_Helper::count($UITypes['related']) > 0) {
    ...
    public function getTranslatedStringCustom($str, $emodule, $language)
    {
        ...
        return ($mod_lang[$str] != '') ? $mod_lang[$str] : (($app_lang[$str] != '') ? $app_lang[$str] : $str);
    }
