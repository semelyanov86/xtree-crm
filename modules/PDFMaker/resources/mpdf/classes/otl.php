...
//echo "DICT_NODE_TYPE_LINEAR: ".dechex($c).'<br />';
				$dictptr++;
				$m = ord(substr($dict,$dictptr,1));
				if ($c == $m) {
					$ptr++;
					if ($ptr > count($this->OTLdata) - 1) {
						$next = ord(substr($dict,$dictptr + 1,1));
...
//echo "DICT_NODE_TYPE_SPLIT ON ".dechex($d).": ".dechex($c).'<br />';
				$dictptr++;
				$d = ord(substr($dict,$dictptr,1));
				if ($c < $d) {
					$dictptr += 5;
				} else {
					$dictptr++;
					// Unsigned long 32-bit offset
					$offset = (ord(substr($dict,$dictptr,1)) * 16777216) + (ord(substr($dict,$dictptr + 1,1)) << 16) + (ord(substr($dict,$dictptr + 2,1)) << 8) + ord(substr($dict,$dictptr + 3,1));
					$dictptr = $offset;
				}
			} else {
//echo "PROBLEM: ".($x).'<br />';
...
// the last of any Marks which immediately follow the current glyph
	...
	function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
	{
        if(empty($level)) {
            $level = 0;
        }
		...
		for ($i = 0; $i < $numchars; ++$i) {
			if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) { // RLE
				...
				if (isset($chunkOTLdata['GPOSinfo'][$i])) {
					...
					$gpos = '';
				$chardata[] = array('char' => $chunkOTLdata['char_data'][$i]['uni'], 'level' => $cel, 'type' => $chardir, 'group' => substr($chunkOTLdata['group'],$i,1), 'GPOSinfo' => $gpos);
			}
		}
	...
	function _bidiReorder(&$chunkorder, &$content, &$cOTLdata, $blockdir)
	{
		...
		for ($nc = 0; $nc < $numchunks; $nc++) {
			...
			for ($i = 0; $i < $numchars; ++$i) {
				...
				$carac['group'] = substr($cOTLdata[$nc]['group'],$i,1);
