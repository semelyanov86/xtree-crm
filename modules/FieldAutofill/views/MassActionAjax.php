...
class FieldAutofill_MassActionAjax_View extends Vtiger_IndexAjax_View
{
    ...
    public function getFieldsOfModules(Vtiger_Request $request)
    {
        ...
        [$primaryModule, $secondaryModule] = explode('_', $selectedVal);
        ...
        $mappings = [];
        $showPopup = 0;
        $rs = $adb->pquery('SELECT * FROM `fieldautofill_mappings` WHERE `key`=? ORDER BY id', [$selectedVal]);
    ...
    public function createNewMapping(Vtiger_Request $request)
    {
        global $adb;
        $selectedVal = $request->get('selected_val');
        $adb->pquery('INSERT INTO `fieldautofill_mappings` (`key`) VALUES (?)', [$selectedVal]);
    ...
    public function saveMappingField(Vtiger_Request $request)
    {
        ...
        $adb->pquery('UPDATE `fieldautofill_mappings` SET `' . $field . '`=? WHERE (`id`=?)', [$fieldname, $mappingId]);
        $response = new Vtiger_Response();
        $response->setEmitType(Vtiger_Response::$EMIT_JSON);
        $response->setResult(['field' => $fieldname]);
        $response->emit();
    }
    ...
    public function delMappingField(Vtiger_Request $request)
    {
        global $adb;
        $mappingId = $request->get('mappingId');
        $adb->pquery('DELETE FROM `fieldautofill_mappings` WHERE (`id`=?)', [$mappingId]);
        $response = new Vtiger_Response();
        $response->setEmitType(Vtiger_Response::$EMIT_JSON);
        $response->setResult(['id' => $mappingId]);
        $response->emit();
    }
}
