                            ...
                            {if $RECORDS_COUNT > 1}
                                <input type="hidden" name="emailSource" value="ListView" />
                                    ...
                                    {/foreach}
                                        {else}
                                            {counter start=0 skip=1 assign="count"} 
                                            {foreach item=EMAIL_MODULE_INFO key=EMAIL_MODULE from=$EMAIL_FIELDS_INFO}
                                                {if $RECORDS_COUNT > 1}<h4>{$RECORD_LABEL}</h4>{/if}
                                                    <div style="{if $RECORDS_COUNT > 1}padding-left: 3%;{/if}">
                                                        {foreach item=EMAIL_FIELDS key=RECORD_ID from=$EMAIL_MODULE_INFO}
                                                            {assign var=RECORD_LABEL value={decode_html(textlength_check(Vtiger_Util_Helper::getRecordName($RECORD_ID)))}}
                                                            <h5>{vtranslate('SINGLE_'|cat:$EMAIL_MODULE, $EMAIL_MODULE)}{$RECORD_LABEL}</h5>
                                                            {foreach item=EMAIL_FIELD  key=EMAIL_VALUE from=$EMAIL_FIELDS}
                                                                <label class="checkbox" style="padding-left: {if $RECORDS_COUNT > 1}10{else}7{/if}%;padding-top: 1%; font-weight:normal;">
                                                                    <input type="checkbox" class="emailField" name="selectedFields[{$count}]" data-moduleName="{$EMAIL_MODULE}" value='{Vtiger_Functions::jsonEncode(['record'=>$RECORD_ID,'field_value'=>$EMAIL_VALUE,'record_label'=>vtranslate('SINGLE_'|cat:$EMAIL_MODULE,$EMAIL_MODULE)|cat:':'|cat:$RECORD_LABEL,'field_id'=> $EMAIL_FIELD->getId(),'module_id'=>$EMAIL_FIELD->getModule()->getId()])}' {if $EMAIL_FIELD->get('isPreferred')}checked="true"{/if}/>
