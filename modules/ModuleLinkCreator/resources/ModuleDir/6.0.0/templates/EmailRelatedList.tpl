            ...
            {foreach item=RELATED_RECORD from=$RELATED_RECORDS}
				...
				{/if}
                            ...
                            {elseif $RELATED_HEADERNAME eq 'parent_id'}
                                ...
                                {assign var=RECORD value=Vtiger_Record_Model::getInstanceById({$smarty.get.record}, $MODULE)}
                                {foreach item=NAME_FIELD from=$MODULE_MODEL->getNameFields()}
                                    {assign var=FIELD_MODEL value=$MODULE_MODEL->getField($NAME_FIELD)}
                                    {if $FIELD_MODEL->getPermissions()}
                                        <a href="index.php?module={$MODULE}&view=Detail&record={$smarty.get.record}" onclick="if(event.stopPropagation){ldelim}event.stopPropagation();{rdelim}else{ldelim}event.cancelBubble=true;{rdelim}" title="Parent ID: {$RECORD->get($NAME_FIELD)}">{$RECORD->get($NAME_FIELD)}</a>
                                    {/if}
                                {/foreach}
