...
var RequestValuesForm2 = function (fieldKey, completeData) {
    ...
    this.show = function(FormHtml, FormScript) {
        ...
        FlexUtils('Workflow2').showModalBox(html).then(jQuery.proxy(function(data) {
            ...
            if(typeof this.completeData['autofocus'] !== 'undefined' && this.completeData['form_settings'] != '') {
                if(jQuery('#wf_startfields').find('[name="' + this.completeData['autofocus'] + '"]').length > 0) {
                    var ele = jQuery('#wf_startfields').find('[name="' + this.completeData['autofocus'] + '"]');

                    if(ele.prop('tagName') == 'INPUT' && (ele.attr('type') == 'text' || ele.attr('type') == 'password')) {
                        ele.select();
                    } else if(ele.prop('tagName') == 'INPUT' && ele.attr('type') == 'hidden') {
                        window.setTimeout(function() {
                            ele.select2('open');
                        }, 600);

                    } else if(ele.prop('tagName') == 'SELECT' || ele.prop('tagName') == 'TEXTAREA') {
                        ele.select();
                    }
                }
            }
