...
if (!$errorMessage) {
    ...
    if (version_compare($vtiger_current_version, '7.0.0') < 0) {
        ...
        $vtigerStandardModules = ['Accounts', 'Assets', 'Calendar', 'Campaigns', 'Contacts', 'CustomerPortal',
        ...
        $result = $db->pquery('SELECT name FROM vtiger_tab WHERE isentitytype != ? AND presence != ? AND trim(name) NOT IN (' . generateQuestionMarks($skipCheckForModules) . ')', [1, 1, $skipCheckForModules]);
        if ($db->num_rows($result)) {
            $moduleModelsList = $extensionStoreInstance->getListings();
        }

        $moduleModelsListByName = [];
        $moduleModelsListByLabel = [];
