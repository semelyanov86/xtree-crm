		 ...
		 <div class="widget_header row-fluid">
			...
			<span class="span10 margin0px">
			 ...
			 {if $WIDGET['data']['action'] eq 1}
				...
				<span class="span2">
					<span class="pull-right">{$RELATED_MODULE_NAME}
                        ...
                        {else}
                            <button class="btn addButton pull-right vteWidgetSendMailButton" type="button" href="javascript:void(0)"
                                    data-url="" data-name="{$RELATED_MODULE_NAME}" onclick='javascript:Vtiger_Detail_Js.triggerSendEmail("index.php?module={$MODULE_NAME}&view=MassActionAjax&mode=showComposeEmailForm&step=step1","Emails");'
                                {if $RELATION_FIELD} data-prf="{$RELATION_FIELD->getName()}" {/if}>
                            <strong>{vtranslate('LBL_ADD',$MODULE_NAME)}</strong>
                        </button>
                        {/if}
