                        ...
                        <tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' data-emailflag='{$EMAIL_FLAG}' name="emailsRelatedRecord">
                            ...
                            {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                                ...
                                <td class="{$WIDTHTYPE}">
                                    ...
                                    {else}
                                        {if $RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME) }
                                            {$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}
                                            {else}
                                            ...
                                            {foreach item=NAME_FIELD from=$MODULE_MODEL->getNameFields()}
                                                ...
                                                {if $FIELD_MODEL->getPermissions()}
                                                    <a href="index.php?module={$MODULE}&view=Detail&record={$smarty.get.record}" title="Parent ID: {$RECORD->get($NAME_FIELD)}">{$RECORD->get($NAME_FIELD)}</a>
                                                {/if}
                                            {/foreach}
                                            {/if}
