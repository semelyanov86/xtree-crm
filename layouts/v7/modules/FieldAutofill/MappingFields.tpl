...
<table class="table table-bordered blockContainer showInlineTable equalSplit" style="width: 500px;">
    <tr>
        <td colspan="3" class="blockHeader" style="background:#fff">
    ...
    {foreach item=mapping key=mapid from=$MAPPINGS}
        <tr data-mapping-id="{$mapid}">
            <td class="fieldValue medium">
                <select class="chzn-select mappingField select2" data-field="primary" style="width: 200px">
                            ...
                            {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
                                <option value="{$FIELD_MODEL->getCustomViewColumnName()}" data-field-name="{$FIELD_NAME}"
                                        {if $FIELD_MODEL->getCustomViewColumnName() eq $mapping['primary']}
                                            selected
                                        {/if}
                                        >{vtranslate($FIELD_MODEL->get('label'), $PRIMODULE_NAME)}
                                </option>
                            {/foreach}
                        </optgroup>
                    {/foreach}
            ...
            <td class="fieldValue medium">
                <select class="chzn-select mappingField select2" data-field="secondary" style="width: 200px">
                    <option value="">{vtranslate('LBL_SELECT_OPTION', 'FieldAutofill')}</option>
            ...
            <td style="text-align: center;line-height:normal;">
                <a class="deleteMappingButton"><i class="glyphicon glyphicon-trash alignMiddle" title="Delete"></i></a>
            </td>
        </tr>
    {/foreach}
    <tr>
    ...
    <tr id="mapping_template" style="display: none;">
        <td class="fieldValue medium">
            <select class="templateselect mappingField select2" data-field="primary" style="width: 200px">
                <option value="">{vtranslate('LBL_SELECT_OPTION', 'FieldAutofill')}</option>
                {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$PRI_RECORD_STRUCTURE}
        ...
        <td class="fieldValue medium">
            <select class="templateselect mappingField select2" data-field="secondary" style="width: 200px">
                <option value="">{vtranslate('LBL_SELECT_OPTION', 'FieldAutofill')}</option>
                {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$SEC_RECORD_STRUCTURE}
                    <optgroup label='{vtranslate($BLOCK_LABEL, $SECMODULE)}'>
                        {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
        ...
        <td style="text-align: center;line-height:normal;">
            <a class="deleteMappingButton"><i class="glyphicon glyphicon-trash alignMiddle" title="{vtranslate('LBL_DELETE', 'FieldAutofill')}"></i></a>
        </td>
    </tr>
</table>