 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger_AdvanceFilter_Js('VTEConditionalAlerts_AdvanceFilter_Js',{},{
	...
	getValues : function() {
		...
		conditionGroups.each(function(index,domElement){
			...
			conditions.each(function(i, conditionDomElement){
                ...
                //Format date to DB format if use mm-dd-yyy or dd-mm-yyyy
                var last2CharOnColumnname = rowValues.columnname.slice(-2);
                if(last2CharOnColumnname == ':D'){
                    if (!isNaN(Date.parse(rowValues['value']))){ 
                        var date_check_value = rowValues['value'];
                        var current_format = jQuery('body').data('user-dateformat');
                        var _delimiter = "-";
                        var arr_date_value = date_check_value.split(_delimiter);
                        if(current_format == 'mm-dd-yyyy'){
                            rowValues['value'] = arr_date_value[2]+ _delimiter + arr_date_value[0] + _delimiter + arr_date_value[1];
                        }
                        if(current_format == 'dd-mm-yyyy'){
                            rowValues['value'] = arr_date_value[2]+ _delimiter + arr_date_value[1] + _delimiter + arr_date_value[0];
                        }
                    }
                }
...
Vtiger_Field_Js('VTEConditionalAlerts_Field_Js',{},{
...
Vtiger_Date_Field_Js('VTEConditionalAlerts_Date_Field_Js',{},{
...
Vtiger_Date_Field_Js('VTEConditionalAlerts_Datetime_Field_Js',{},{
	/**
	 * Function to get the user date format
	 */
	getDateFormat : function(){
...
Vtiger_Currency_Field_Js('VTEConditionalAlerts_Currency_Field_Js',{},{
...
Vtiger_Time_Field_Js('VTEConditionalAlerts_Time_Field_Js',{},{
...
Vtiger_Field_Js('Vtiger_Boolean_Field_Js',{},{
	 ...
	 */
	getUi : function() {
		var html = '<input type="text" class="getPopupUi boolean form-control" name="'+ this.getName() +'" value="'+  this.getValue() + '" />'+
					'<input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />';
		var element = jQuery(html);
		return this.addValidationToElement(element);
	}
...
Vtiger_Owner_Field_Js('VTEConditionalAlerts_Owner_Field_Js',{},{

    getUi : function() {
		var html = '<select class="row chzn-select select2" name="'+ this.getName() +'" style="min-width: 200px">';
...
Vtiger_Picklist_Field_Js('VTEConditionalAlerts_Picklist_Field_Js',{},{
        getUi : function(){
                ...
                var html = '<input type="hidden" class="row select2" style="min-width: 200px;" name="'+ this.getName() +'">';
                ...
                // });
				vtUtils.applyFieldElementsView(selectContainer);
