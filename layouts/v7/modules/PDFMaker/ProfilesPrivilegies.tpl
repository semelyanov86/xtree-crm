...
<div class="container-fluid">
    ...
    <br>
    <label class="pull-left themeTextColor font-x-x-large">{vtranslate('LBL_PROFILES','PDFMaker')}</label>
    {if $MODE neq "edit"}<button class="btn pull-right" type="submit">{vtranslate('LBL_EDIT','PDFMaker')}</button>{/if}
    <br clear="all">{vtranslate('LBL_PROFILES_DESC','PDFMaker')}
    <hr>
    ...
    <br />
    <div class="row-fluid">
        <label class="fieldLabel"><strong>{vtranslate('LBL_SETPRIVILEGIES','PDFMaker')}:</strong></label><br>
        <table class="table table-striped table-bordered profilesEditView">
            <thead>
                <tr class="blockHeader">
                    <th style="border-left: 1px solid #DDD !important;" width="40%">{vtranslate('LBL_PROFILES','PDFMaker')}</th>
                    <th style="border-left: 1px solid #DDD !important;" width="15%" align="center">{vtranslate('LBL_CREATE_EDIT','PDFMaker')}</th>
                    <th style="border-left: 1px solid #DDD !important;" width="15%" align="center">{vtranslate('LBL_VIEW','PDFMaker')}</th>
                    <th style="border-left: 1px solid #DDD !important;" width="15%" align="center">{vtranslate('LBL_DELETE','PDFMaker')}</th>
                    <th style="border-left: 1px solid #DDD !important;" width="15%" align="center" class="{if $IS_RTF_ACTIVATED eq "no"}hide{/if}">{vtranslate('LBL_EXPORT_TO_RTF','PDFMaker')}</th>
                </tr>
            </thead>
            <tbody>
            {foreach item=arr from=$PERMISSIONS}
                {foreach key=profile_name item=profile_arr from=$arr}
                    <tr>
                        ...
                        <td class="cellText {if $IS_RTF_ACTIVATED eq "no"}hide{/if}" align="center">
                            {if $MODE eq "edit"}
                                <input type="checkbox" {$profile_arr.EXPORT_RTF.checked} id="{$profile_arr.EXPORT_RTF.name}" name="{$profile_arr.EXPORT_RTF.name}" onclick="other_chk_clicked(this, '{$profile_arr.DETAIL.name}');"/>
                            {else}    
                                <img style="margin-left: 40%" class="alignMiddle" src="{if $profile_arr.EXPORT_RTF.checked neq ""}{$ENABLE_IMAGE_PATH}{else}{$DISABLE_IMAGE_PATH}{/if}" />
                            {/if}
                        </td>
                    </tr>
                {/foreach}    
