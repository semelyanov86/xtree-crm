		...
		<table class="table table-bordered table-condensed listViewEntriesTable">
			<thead>
				<tr class="listViewHeaders">
					...
					<th>{vtranslate('LBL_TASK_TITLE','VTEConditionalAlerts')}</th>
				</tr>
			</thead>
			<tbody>
				{foreach from=$TASK_LIST item=TASK}
					<tr class="listViewEntries">
						...
						<td><a data-url="?module=VTEConditionalAlerts&parent=Settings&view=EditTask&selected_module={$SELECTED_MODULE}&cat_id={$TASK['cat_id']}&task_id={$TASK['id']}">{$TASK['title']}</a>
							<div class="pull-right actions">
								<span class="actionImages">
									<a data-url="?module=VTEConditionalAlerts&parent=Settings&view=EditTask&selected_module={$SELECTED_MODULE}&cat_id={$TASK['cat_id']}&task_id={$TASK['id']}">
										<i class="icon-pencil alignMiddle" title="{vtranslate('LBL_EDIT','VTEConditionalAlerts')}"></i>
									</a>&nbsp;&nbsp;
									<a class="deleteTask" data-id="{$TASK['id']}" data-deleteurl="{$TASK['remove_link']}">
										<i class="icon-trash alignMiddle" title="{vtranslate('LBL_DELETE','VTEConditionalAlerts')}"></i>
									</a>
								</span>
							</div>
						</td>
